...
- [**cloudbox**](https://github.com/acoyfellow/cloudbox) — synthetic cloud computers for training and evaluating long-horizon agents. End-to-end on Cloudflare: persona → filesystem → artifacts → collaborators → simulation → retrospective.
- [**deja**](https://github.com/acoyfellow/deja) — persistent semantic memory for agents. Two functions, ~500 lines. The agent stays stateless; the system remembers forever.
- [**desk**](https://github.com/acoyfellow/desk) — a personal app store for tiny edge devices. `git push` installs apps; MCP lets agents use the device as an out-of-band physical I/O surface.
- [**terrarium**](https://github.com/acoyfellow/terrarium) — one-level orchestration harness for composable AI subagents.

## writing

A handful from [coey.dev/blog](https://coey.dev/blog) — start with these.

- [**Patience: You've Never Controlled Anything**](https://coey.dev/patience) — what Memento taught me about prompting agents.
- [**Campfire: The Oldest Protocol**](https://coey.dev/campfire) — every storytelling medium had a context window. We've always been writing prompts.
- [**Prompts Are Wishes**](https://coey.dev/prompts-are-wishes) — every prompt is a hypothesis. The guardrails are somewhere else entirely.
- [**Liquid Primitives**](https://coey.dev/liquid-primitives) — five parts of your stack to hold loosely. Harnesses, models, source code, docs, prompts.
- [**Compaction**](https://coey.dev/compaction) — 75 API calls, 5 conditions. Goals survive every form of compaction. Reasoning never does.
...
- [living-artifact](https://github.com/acoyfellow/living-artifact) — physical/digital artifacts and tiny devices.
